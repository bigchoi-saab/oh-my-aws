...
1. **US-0 parity baseline 확인**:
   ...
   - Reference: `.kiro/skills/aws-exam/references/us0-parity-runbook.md`
               `.kiro/skills/aws-exam/references/live-incident-bootstrap-guide.md`
2. **scripts/ 디렉토리 확인**: `.kiro/skills/aws-exam/scripts/audit_bash_calls.py`, `audit_dkr_access.py` 존재 여부
   - 없으면 오류 출력 후 중단
3. **harness/ 확인**: `.kiro/skills/aws-exam/harness/harness.py` 존재 여부
   - 없으면 오류 출력 후 중단
4. **실행 모드 결정**:
...
1. `.kiro/skills/aws-incident-response/references/exam-bank/_index.yaml` 로드 → 출제 범위 확인
...
> **[중요] DKR 접근 차단**: 이 Phase에서 `.kiro/skills/aws-incident-response/references/scenarios/` 하위 어떤 파일도 읽지 않는다.
...
3. **aws-diagnostician 페르소나 spawn** (simulation mode):
   - **Kiro agent 사용 원칙**: Kiro에서는 `.kiro/agents/aws-diagnostician.json`이 canonical agent config이고, `.kiro/agents/aws-diagnostician.md`는 그 agent가 참조하는 prompt 본문이다. sim mode에서는 기존 페르소나를 유지하되, `exam-simulation-protocol.md` 규칙이 더 높은 우선순위로 적용된다.
   - spawn prompt 조립 순서 (위에서 아래로):
     ...
     3. `Read('.kiro/agents/aws-diagnostician.md')` 결과를 `BASE AGENT PERSONA (precedence: lower — inform style, not workflow)` 블록으로 삽입
   ...
   - Agent invocation target:
     - prefer the Kiro custom agent `aws-diagnostician`
     - if direct agent selection is unavailable in the current runtime, inject the same prompt into the nearest equivalent worker agent without relaxing the sim-mode guardrails
...
5. **Bash 호출 감사**: `.kiro/skills/aws-exam/scripts/audit_bash_calls.py` 실행
...
> **참조**: `.kiro/skills/aws-exam/references/reflection-protocol.md`
...
5. **반성기는 `.kiro/` 하위 어떤 런타임 자산도 직접 편집하지 않는다** — 제안만 생성
...
See full protocol: `.kiro/skills/aws-exam/references/approval-gate.md`
...
**CRITICAL**: Auto-editing of any Kiro runtime file under `.kiro/` is explicitly forbidden at every branch.
...
- **자동 편집 금지**: Phase 6에서 `.kiro/` 런타임 자산 자동 편집 절대 금지
